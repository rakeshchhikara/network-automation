...
def run_cmd(user_cmd, hostname, connection):
...
def retrieve_file(device_ip, port, username, password, filename):
...
def remote_md5_check(connection, filename):
...
def md5_compare(remote_md5, md5_local, local_files):
...
def run_cmd_admin(user_cmd, hostname, connection, active_processor):
...
def upload_2_sr(local_files, sr_username, sr_token, url):
...
try:
    if get_choice == 1:
        ...
        for user_cmd in lines:
            filename = run_cmd(user_cmd, hostname, connection)
            if filename is None:
                continue
            remote_md5 = remote_md5_check(connection, filename)
            local_file_name = retrieve_file(device_ip, '22', username, password, filename)
            md5_local = local_md5_check(filename)
            md5_compare(remote_md5, md5_local, local_files)

        print(f'List of local files: {local_files}')

        upload_2_sr(local_files, sr_username, sr_token, url)

    elif get_choice == 2:
        print("\nuser selected option: 2\n")
        lines = []
        while True:
            line = input('Enter command (only admin mode commands without admin keyword): ')
            if line:
                lines.append(line)
            else:
                break
        ...
        for user_cmd in lines:
            filename = run_cmd_admin(user_cmd, hostname, connection, active_processor)
            if filename is None:
                continue
            remote_md5 = remote_md5_check(connection, filename)
            local_file_name = retrieve_file(device_ip, '22', username, password, filename)
        ...
        lines = []
        while True:
            line = input('Enter filename with full-path: ')
            if line:
                lines.append(line)
            else:
                break
